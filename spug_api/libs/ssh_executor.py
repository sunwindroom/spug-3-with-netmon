...
优化点（本次）：
  * ssh_exec 增加可选重试参数，网络抖动导致的瞬时连接失败可自动重试。
  * ping_check 增加 subprocess 超时(timeout+2s)，防止 ping 进程在异常网络环境下挂起。
  * ssh_exec_ok 透传重试参数。
"""
import subprocess
import platform
...
def ssh_exec(host, command, retries=1):
    ...
    for attempt in range(retries + 1):
        try:
            ...
            last_error = e
            logger.warning('SSH执行异常 host=%s attempt=%d: %s', host.hostname, attempt + 1, e)
    return -1, f'异常信息：{last_error}'


def ssh_exec_ok(host, command, retries=1):
    """ssh_exec 的便捷封装：执行成功返回 (True, stdout)，失败返回 (False, 错误信息)。"""
    exit_code, out = ssh_exec(host, command, retries=retries)
    if exit_code == 0:
