...
def monitor_worker_handler(job):
    ...
    if tp == '1':
        ...
        if not host:
            ...
            is_ok, message = host_executor(host, f'ps -ef|grep -v grep|grep {extra!r}')
        elif tp == '4':
            is_ok, message = host_executor(host, extra)
        elif tp == '6':
            is_ok, message = docker_check(host, extra)
        else:  # tp == '8'
            path, _, keyword = (extra or '').partition('||')
            is_ok, message = log_check(host, path, keyword)
        target = f'{host.name}({host.hostname})' if host else addr
...
def dispatch(tp, addr, extra):
    if tp == '1':
        ...
        return docker_check(host, extra)
    elif tp == '8':
        host = Host.objects.filter(pk=addr).first()
        if not host:
            return False, f'unknown host id for {addr!r}'
        path, _, keyword = (extra or '').partition('||')
        return log_check(host, path, keyword)
    else:
